...
/* --- Emergency Reentry (shared) ---------------------------------------- */

/* The four historical "EMERGENCY REENTRY" call sites, as parameter presets. */
const RescueParams RESCUE_PARAMS_STANDARD = {
    .sector_offset = QUADRANT_SIZE / 8.0,
    .full_torpedo_reload = false,
    .always_reset_crew = false,
    .force_shutdown = false,
    .reset_motion = true,
    .reactivate = false
};
const RescueParams RESCUE_PARAMS_CREW = {
    .sector_offset = QUADRANT_SIZE / 8.0,
    .full_torpedo_reload = false,
    .always_reset_crew = true,
    .force_shutdown = false,
    .reset_motion = true,
    .reactivate = false
};
const RescueParams RESCUE_PARAMS_TACTICAL = {
    .sector_offset = QUADRANT_SIZE / 8.0,
    .full_torpedo_reload = false,
    .always_reset_crew = true,
    .force_shutdown = true,
...
const RescueParams RESCUE_PARAMS_LOGIN = {
    .sector_offset = QUADRANT_SIZE / 2.0,
    .full_torpedo_reload = true,
    .always_reset_crew = false,
    .force_shutdown = false,
    .reset_motion = false,
    .reactivate = true
...
/*
 * EMERGENCY REENTRY PROTOCOL: relocate the player's hull to a random safe
 * quadrant and restore escape-vessel state.
 *
 * When seed is non-NULL, rand_r(seed) is used (deterministic tick seed, as in
 * the game loop); otherwise rand() is used (command / sync threads).
 */
void rescue_player(int i, const RescueParams *p, unsigned int *seed) {
    int rq1, rq2, rq3;
    do {
        rq1 = seed ? rand_r(seed) % GALAXY_SIZE + 1 : rand() % GALAXY_SIZE + 1;
        rq2 = seed ? rand_r(seed) % GALAXY_SIZE + 1 : rand() % GALAXY_SIZE + 1;
        rq3 = seed ? rand_r(seed) % GALAXY_SIZE + 1 : rand() % GALAXY_SIZE + 1;
    ...
    players[i].state.s1 = p->sector_offset;
    players[i].state.s2 = p->sector_offset;
    players[i].state.s3 = p->sector_offset;
    players[i].state.energy = MAX_ENERGY_CAPACITY; /* Standard capacity */
    players[i].state.torpedoes = p->full_torpedo_reload ? MAX_TORPEDO_CAPACITY : (MAX_TORPEDO_CAPACITY / 10);
    if (p->always_reset_crew || players[i].state.crew_count <= 0) {
    ...
    players[i].gx = (rq1 - 1) * QUADRANT_SIZE + p->sector_offset;
    players[i].gy = (rq2 - 1) * QUADRANT_SIZE + p->sector_offset;
    players[i].gz = (rq3 - 1) * QUADRANT_SIZE + p->sector_offset;
...
void update_game_logic() {
        ...
        if (players[i].socket && players[i].death_timer > 0) {
            ...
            if (players[i].death_timer <= 0) {
                #pragma omp critical
                {
                    ...
                    rescue_player(i, &RESCUE_PARAMS_STANDARD, &ai_seed);
    ...
    for (int i = 0; i < MAX_CLIENTS; i++) {
        ...
        double old_gx = players[i].gx;
        double old_gy = players[i].gy;
        double old_gz = players[i].gz;
        ...
        if (players[i].state.hull_integrity <= 0.0) {
            ...
            if (players[i].death_timer <= 0) {
                #pragma omp critical
                {
                    ...
                    slog("\033[1;31m[DESTRUCTION]\033[0m Captain \033[1;37m%-15s\033[0m ship has been destroyed. [\033[1;33m%s\033[0m]\n", 
        ...
        if (players[i].state.crew_count <= 0) {
            #pragma omp critical
            {
                ...
                rescue_player(i, &RESCUE_PARAMS_CREW, &ai_seed);
        ...
        if (players[i].state.lock_target > 0) {
            ...
            if (pq1 != players[i].last_q1 || pq2 != players[i].last_q2 || pq3 != players[i].last_q3) {
                ...
                /* All static object types: single table-driven lookup (targets.c) */
                valid = target_is_active_in_quadrant(tid, pq1, pq2, pq3);
            }
        ...
        if (players[i].nav_state == NAV_STATE_ALIGN || players[i].nav_state == NAV_STATE_ALIGN_IMPULSE) {
             ...
             * turn across the 0/360 boundary never leaves the range
             * (B2: the HUD must not display headings above 360). */
            ...
            if (tid >= GALAXY_OBJECT_MIN_PLAYER && tid <= GALAXY_OBJECT_MAX_PLAYER) { 
                if (tid <= MAX_CLIENTS && players[tid - 1].active) {
                    tx = players[tid - 1].gx;
                    ty = players[tid - 1].gy;
                    tz = players[tid - 1].gz;
                    found = true;
                ...
                /* Table-driven resolution (targets.c): local spatial-index
                   lookup first, then the cross-quadrant global fallback. */
                const TargetRangeDef *tr = target_range_for(tid);

                /* 1. Local objects check (Fast path) */
                if (tr && (tr->flags & TGT_F_APR_LOCAL)) {
                    const void *obj = target_find_local(tr, lq, tid);
                    if (obj) {
                        target_abs_pos(tr, obj, &tx, &ty, &tz);
                        found = true;
                    }
                }

                /* 2. Global fallback (Cross-quadrant tracking) */
                if (!found && tr && (tr->flags & TGT_F_APR_GLOBAL)) {
                    const void *obj = target_find_global_active(tid);
                    if (obj) {
                        target_abs_pos(tr, obj, &tx, &ty, &tz);
                        found = true;
                    }
                ...
                /* 3. Player probes live in the player state, not the index */
            ...
            if (tid >= GALAXY_OBJECT_MIN_PLAYER && tid <= GALAXY_OBJECT_MAX_PLAYER) {
                ...
                /* Table-driven global lookup (targets.c): comets & monsters */
                const TargetRangeDef *tr = target_range_for(tid);
                if (tr && (tr->flags & TGT_F_CHASE)) {
                    const void *obj = target_find_global_active(tid);
                    if (obj) {
                        target_abs_pos(tr, obj, &tx, &ty, &tz);
                        found = true;
...
#pragma omp parallel for schedule(dynamic, 10)
    ...
    for (int i = 0; i < MAX_CLIENTS; i++) {
        ...
        for(int n=0; n<lq->npc_count && o_idx < MAX_NET_OBJECTS; n++) {
            ...
            upd->objects[o_idx] = (NetObject){npc->x, npc->y, npc->z, npc->h, npc->m, 0.0, npc->faction, npc->ship_class, 1, (int)(npc->health / (int)THRESHOLD_SYS_CRITICAL), npc->energy, npc->plating, (int)(npc->health / (int)THRESHOLD_SYS_CRITICAL), npc->faction, npc->id + GALAXY_OBJECT_MIN_NPC, npc->is_cloaked, "", npc->vx, npc->vy, npc->vz};
        ...
        for(int j=0; j<lq->player_count && o_idx < MAX_NET_OBJECTS; j++) {
            ...
            upd->objects[o_idx] = (NetObject){p->state.s1, p->state.s2, p->state.s3, p->state.van_h, p->state.van_m, p->state.van_r, 1, p->ship_class, 1, (int)p->state.hull_integrity, p->state.energy, 0, (int)p->state.hull_integrity, p->faction, (int)(p - players) + GALAXY_OBJECT_MIN_PLAYER, p->state.is_cloaked, "", p->vx, p->vy, p->vz};
        ...
        for(int a=0; a<lq->artifact_count && o_idx < MAX_NET_OBJECTS; a++) { NPCArtifact *obj = lq->artifacts[a]; if(!obj->active) continue; upd->objects[o_idx++] = (NetObject){obj->x, obj->y, obj->z, 0, 0, 0, 40, 0, 1, 100, 0, 0, 100, 40, obj->id + GALAXY_OBJECT_MIN_ARTIFACT, 0, "Alien Artifact", 0,0,0}; }
        for(int a=0; a<lq->warp_gate_count && o_idx < MAX_NET_OBJECTS; a++) { NPCWarpGate *obj = lq->warp_gates[a]; if(!obj->active) continue; upd->objects[o_idx++] = (NetObject){obj->x, obj->y, obj->z, 0, 0, 0, 41, 0, 1, 100, 0, 0, 100, 41, obj->id + GALAXY_OBJECT_MIN_WARP_GATE, 0, "Warp Gate", 0,0,0}; }
        for(int a=0; a<lq->neutron_star_count && o_idx < MAX_NET_OBJECTS; a++) { NPCNeutronStar *obj = lq->neutron_stars[a]; if(!obj->active) continue; upd->objects[o_idx++] = (NetObject){obj->x, obj->y, obj->z, 0, 0, 0, 42, 0, 1, 100, 0, 0, 100, 42, obj->id + GALAXY_OBJECT_MIN_NEUTRON_STAR, 0, "Neutron Star", 0,0,0}; }
        for(int a=0; a<lq->mega_struct_count && o_idx < MAX_NET_OBJECTS; a++) { NPCMegaStructure *obj = lq->mega_structs[a]; if(!obj->active) continue; upd->objects[o_idx++] = (NetObject){obj->x, obj->y, obj->z, 0, 0, 0, 43, 0, 1, 100, 0, 0, 100, 43, obj->id + GALAXY_OBJECT_MIN_MEGA_STRUCT, 0, "Mega Structure", 0,0,0}; }
        for(int a=0; a<lq->dark_cloud_count && o_idx < MAX_NET_OBJECTS; a++) { NPCDarkCloud *obj = lq->dark_clouds[a]; if(!obj->active) continue; upd->objects[o_idx++] = (NetObject){obj->x, obj->y, obj->z, 0, 0, 0, 44, 0, 1, 100, 0, 0, 100, 44, obj->id + GALAXY_OBJECT_MIN_DARK_CLOUD, 0, "Dark Matter Cloud", 0,0,0}; }
        for(int a=0; a<lq->singularity_count && o_idx < MAX_NET_OBJECTS; a++) { NPCSingularity *obj = lq->singularities[a]; if(!obj->active) continue; upd->objects[o_idx++] = (NetObject){obj->x, obj->y, obj->z, 0, 0, 0, 45, 0, 1, 100, 0, 0, 100, 45, obj->id + GALAXY_OBJECT_MIN_SINGULARITY, 0, "Quantum Singularity", 0,0,0}; }
        for(int a=0; a<lq->plasma_storm_count && o_idx < MAX_NET_OBJECTS; a++) { NPCPlasmaStorm *obj = lq->plasma_storms[a]; if(!obj->active) continue; upd->objects[o_idx++] = (NetObject){obj->x, obj->y, obj->z, 0, 0, 0, 46, 0, 1, 100, 0, 0, 100, 46, obj->id + GALAXY_OBJECT_MIN_PLASMA_STORM, 0, "Plasma Storm", 0,0,0}; }
        for(int a=0; a<lq->orbital_ring_count && o_idx < MAX_NET_OBJECTS; a++) { NPCOrbitalRing *obj = lq->orbital_rings[a]; if(!obj->active) continue; upd->objects[o_idx++] = (NetObject){obj->x, obj->y, obj->z, 0, 0, 0, 47, 0, 1, 100, 0, 0, 100, 47, obj->id + GALAXY_OBJECT_MIN_ORBITAL_RING, 0, "Orbital Ring", 0,0,0}; }
        for(int a=0; a<lq->time_anomaly_count && o_idx < MAX_NET_OBJECTS; a++) { NPCTimeAnomaly *obj = lq->time_anomalies[a]; if(!obj->active) continue; upd->objects[o_idx++] = (NetObject){obj->x, obj->y, obj->z, 0, 0, 0, 48, 0, 1, 100, 0, 0, 100, 48, obj->id + GALAXY_OBJECT_MIN_TIME_ANOMALY, 0, "Time Anomaly", 0,0,0}; }
        for(int a=0; a<lq->void_crystal_count && o_idx < MAX_NET_OBJECTS; a++) { NPCVoidCrystal *obj = lq->void_crystals[a]; if(!obj->active) continue; upd->objects[o_idx++] = (NetObject){obj->x, obj->y, obj->z, 0, 0, 0, 49, 0, 1, 100, 0, 0, 100, 49, obj->id + GALAXY_OBJECT_MIN_VOID_CRYSTAL, 0, "Void Crystal", 0,0,0}; }
        for(int a=0; a<lq->subspace_anomaly_count && o_idx < MAX_NET_OBJECTS; a++) { NPCSubspaceAnomaly *obj = lq->subspace_anomalies[a]; if(!obj->active) continue; upd->objects[o_idx++] = (NetObject){obj->x, obj->y, obj->z, 0, 0, 0, 50, 0, 1, 100, 0, 0, 100, 50, obj->id + GALAXY_OBJECT_MIN_SUBSPACE_ANOM, 0, "Subspace Anomaly", 0,0,0}; }
        ...
        for(int c=0; c<lq->comet_count && o_idx < MAX_NET_OBJECTS; c++) { NPCComet *co = lq->comets[c]; if(!co->active) continue; upd->objects[o_idx++] = (NetObject){co->x, co->y, co->z, co->h, co->m, 0, 9, 0, 1, 100, 0, 0, 100, 9, co->id + GALAXY_OBJECT_MIN_COMET, 0, "Comet", 0,0,0}; }
        ...
        for(int a=0; a<lq->asteroid_count && o_idx < MAX_NET_OBJECTS; a++) { NPCAsteroid *as = lq->asteroids[a]; if(!as->active) continue; upd->objects[o_idx++] = (NetObject){as->x, as->y, as->z, 0, 0, 0, 21, as->resource_type, 1, 100, as->amount, (int)(as->size * 100), 100, 21, as->id + GALAXY_OBJECT_MIN_ASTEROID, 0, "Asteroid", 0,0,0}; }
        for(int m=0; m<lq->mine_count && o_idx < MAX_NET_OBJECTS; m++) { NPCMine *mi = lq->mines[m]; if(!mi->active) continue; upd->objects[o_idx++] = (NetObject){mi->x, mi->y, mi->z, 0, 0, 0, 23, 0, 1, 100, 0, 0, 100, 23, mi->id + GALAXY_OBJECT_MIN_MINE, 0, "Mine", 0,0,0}; }
        for(int b=0; b<lq->buoy_count && o_idx < MAX_NET_OBJECTS; b++) { NPCBuoy *bu = lq->buoys[b]; if(!bu->active) continue; upd->objects[o_idx++] = (NetObject){bu->x, bu->y, bu->z, 0, 0, 0, 24, 0, 1, 100, 0, 0, 100, 24, bu->id + GALAXY_OBJECT_MIN_BUOY, 0, "Comm Buoy", 0,0,0}; }
