...
void handle_enc(int i, const char *params) {
    ...
    if (is_pqc) {
        cost = 250;
        if (players[i].state.system_health[6] < 50.0f) {
    ...
    if (sscanf(params, "%f %f %f", &e, &s, &w) == 3) {
        if (players[i].state.system_health[6] < 10.0f) {
            ...
            return;
                ...
                if (players[i].state.system_health[6] < 50.0f && (rand() % 100 < 30)) {
...
void handle_imp(int i, const char *params) {
    ...
    if (players[i].state.system_health[1] < 40.0f && (rand() % 100 > (int)players[i].state.system_health[1])) {
    ...
    if (args == 1) {
        ...
        send_server_msg(i, "HELMSMAN", msg); players[i].nav_state = NAV_STATE_IMPULSE; players[i].state.energy -= 50;
    } else if (args == 3) {
        normalize_upright(&h, &m);
        players[i].target_h = h; players[i].target_m = m;
        players[i].start_h = players[i].state.van_h; players[i].start_m = players[i].state.van_m;
        ...
        players[i].nav_state = NAV_STATE_ALIGN_IMPULSE; players[i].state.energy -= 100;
        double dh = players[i].target_h - players[i].state.van_h;
        while(dh>180) dh-=360; 
        while(dh<-180) dh+=360;
        players[i].nav_timer = (fabs(dh)<1.0 && fabs(players[i].target_m - players[i].state.van_m)<1.0) ? 10 : 60;
        send_server_msg(i, "HELMSMAN", "Course plotted. Aligning for Impulse drive.");
    } else send_server_msg(i, "COMPUTER", "Usage: imp <H> <M> <S> or imp <S>");
}
...
void handle_apr(int i, const char *params) {
    ...
    if (players[i].state.system_health[1] < 10.0f || players[i].state.system_health[6] < 10.0f) {
    ...
    else if (args == 1 && tid < 100) { tdist = (double)tid; tid = players[i].state.lock_target; }
    ...
    if (tid > 0) {
        ...
        /* Extensive Target Resolution Logic */
        if (tid >= 1 && tid <= 32) {
            ...
            if (tid >= 2000 && tid < 2000+MAX_BASES) { for(int b=0; b<lq->base_count; b++) if(lq->bases[b]->id+2000 == tid) { tx = (lq->bases[b]->q1-1)*10+lq->bases[b]->x; ty = (lq->bases[b]->q2-1)*10+lq->bases[b]->y; tz = (lq->bases[b]->q3-1)*10+lq->bases[b]->z; found = true; strcpy(target_name, "Starbase"); } }
            else if (tid >= 3000 && tid < 3000+MAX_PLANETS) { for(int p=0; p<lq->planet_count; p++) if(lq->planets[p]->id+3000 == tid) { tx = (lq->planets[p]->q1-1)*10+lq->planets[p]->x; ty = (lq->planets[p]->q2-1)*10+lq->planets[p]->y; tz = (lq->planets[p]->q3-1)*10+lq->planets[p]->z; found = true; strcpy(target_name, "Planet"); } }
            else if (tid >= 4000 && tid < 4000+MAX_STARS) { for(int s=0; s<lq->star_count; s++) if(lq->stars[s]->id+4000 == tid) { tx = (lq->stars[s]->q1-1)*10+lq->stars[s]->x; ty = (lq->stars[s]->q2-1)*10+lq->stars[s]->y; tz = (lq->stars[s]->q3-1)*10+lq->stars[s]->z; found = true; strcpy(target_name, "Star"); } }
            else if (tid >= 7000 && tid < 7000+MAX_BH) { for(int h=0; h<lq->bh_count; h++) if(lq->black_holes[h]->id+7000 == tid) { tx = (lq->black_holes[h]->q1-1)*10+lq->black_holes[h]->x; ty = (lq->black_holes[h]->q2-1)*10+lq->black_holes[h]->y; tz = (lq->black_holes[h]->q3-1)*10+lq->black_holes[h]->z; found = true; strcpy(target_name, "Black Hole"); } }
            else if (tid >= 10000 && tid < 10000+MAX_COMETS) { for(int c=0; c<lq->comet_count; c++) if(lq->comets[c]->id+10000 == tid) { tx = (lq->comets[c]->q1-1)*10+lq->comets[c]->x; ty = (lq->comets[c]->q2-1)*10+lq->comets[c]->y; tz = (lq->comets[c]->q3-1)*10+lq->comets[c]->z; found = true; strcpy(target_name, "Comet"); } }
            else if (tid >= 11000 && tid < 11000+MAX_DERELICTS) { for(int d=0; d<lq->derelict_count; d++) if(lq->derelicts[d]->id+11000 == tid) { tx = (lq->derelicts[d]->q1-1)*10+lq->derelicts[d]->x; ty = (lq->derelicts[d]->q2-1)*10+lq->derelicts[d]->y; tz = (lq->derelicts[d]->q3-1)*10+lq->derelicts[d]->z; found = true; strcpy(target_name, "Derelict"); } }
            else if (tid >= 12000 && tid < 12000+MAX_ASTEROIDS) { for(int a=0; a<lq->asteroid_count; a++) if(lq->asteroids[a]->id+12000 == tid) { tx = (lq->asteroids[a]->q1-1)*10+lq->asteroids[a]->x; ty = (lq->asteroids[a]->q2-1)*10+lq->asteroids[a]->y; tz = (lq->asteroids[a]->q3-1)*10+lq->asteroids[a]->z; found = true; strcpy(target_name, "Asteroid"); } }
            else if (tid >= 14000 && tid < 14000+MAX_MINES) { for(int m=0; m<lq->mine_count; m++) if(lq->mines[m]->id+14000 == tid) { tx = (lq->mines[m]->q1-1)*10+lq->mines[m]->x; ty = (lq->mines[m]->q2-1)*10+lq->mines[m]->y; tz = (lq->mines[m]->q3-1)*10+lq->mines[m]->z; found = true; strcpy(target_name, "Mine"); } }
            else if (tid >= 15000 && tid < 15000+MAX_BUOYS) { for(int b=0; b<lq->buoy_count; b++) if(lq->buoys[b]->id+15000 == tid) { tx = (lq->buoys[b]->q1-1)*10+lq->buoys[b]->x; ty = (lq->buoys[b]->q2-1)*10+lq->buoys[b]->y; tz = (lq->buoys[b]->q3-1)*10+lq->buoys[b]->z; found = true; strcpy(target_name, "Comm Buoy"); } }
        ...
        } else send_server_msg(i, "COMPUTER", "Target not found or obscured.");
    } else send_server_msg(i, "COMPUTER", "Usage: apr <ID> [DIST]");
}

void handle_xxx(int i, const char *params) {
    players[i].active = 0; send_server_msg(i, "COMPUTER", "Self-destruct sequence complete. BOOM.");
}

void handle_cha(int i, const char *params) {
    if (players[i].state.system_health[1] < 10.0f || players[i].state.system_health[6] < 10.0f) { send_server_msg(i, "COMPUTER", "CHASE FAILURE."); return; }
...
void handle_srs(int i, const char *params) {
    if (players[i].state.system_health[2] < 5.0f) {
    ...
    for(int b_idx=0; b_idx<local_q->base_count; b_idx++) {
        ...
        SAFE_APPEND(b, LARGE_DATA_BUFFER, "🛰️  %-10s %-5d [%.1f,%.1f,%.1f] %-5.1f %03.0f / %+03.0f     Alliance Starbase %s\n", "Starbase", baid, ba->x, ba->y, ba->z, d, h, m, status);
    }
    ...
    for(int h_idx=0; h_idx<local_q->bh_count; h_idx++) {
        ...
        SAFE_APPEND(b, LARGE_DATA_BUFFER, "🕳️  %-10s %-5d [%.1f,%.1f,%.1f] %-5.1f %03.0f / %+03.0f     Black Hole (Gravity Well)\n", "B-Hole", bh->id+7000, bh->x, bh->y, bh->z, d, hh, m);
    }
...
void handle_lrs(int i, const char *params) {
    if (players[i].state.system_health[2] < 15.0f) {
    ...
    if (players[i].state.system_health[2] < 50.0f) {
        ...
        for (int dq2 = -scan_range; dq2 <= scan_range; dq2++) {
            for (int dq1 = -scan_range; dq1 <= scan_range; dq1++) {
                ...
                if (IS_Q_VALID(nq1, nq2, nq3)) {
                    ...
                    float sensor_h = players[i].state.system_health[2];
                    if (sensor_h < 50.0f && (rand()%100 > sensor_h)) v = (v / 10) + (rand()%9);
...
void handle_pha(int i, const char *params) {
    ...
    if (players[i].state.system_health[4] < 10.0f) {
    ...
    if (players[i].state.ion_beam_charge < 10.0f) { send_server_msg(i, "TACTICAL", "Banks recharging."); return; }
    ...
    float sensor_h = players[i].state.system_health[2];
    if (sensor_h < 50.0f && (rand() % 100 > (int)(sensor_h + 25))) {
        players[i].state.energy -= (e / 2);
        players[i].state.ion_beam_charge -= 10.0f;
    ...
    if (found) {
        ...
        if (tid <= 32) {
            ...
            if (dmg_rem > 0) {
                target->state.hull_integrity -= (dmg_rem / 1000.0f);
                if (rand() % 100 < 20) {
                    int sys = rand() % 10; target->state.system_health[sys] -= (5.0f + rand() % 15);
                    send_server_msg(tid-1, "DAMAGE", "Hull impact! Internal system damage!");
                }
...
void handle_she(int i, const char *params) {
    ...
    if (sscanf(params, "%d %d %d %d %d %d", &f, &r, &t, &b, &l, &ri) == 6) {
        ...
        if (players[i].state.system_health[8] < 10.0f) {
        ...
        if (diff > 0) {
            ...
            players[i].state.energy -= 50;
            players[i].state.energy += (int)(abs(diff) * 0.8f);
...
void handle_lock(int i, const char *params) {
    ...
    if (sscanf(params, " %d", &tid) == 1) {
        ...
        if (players[i].state.system_health[2] < 10.0f) {
        ...
        if (tid >= 1 && tid <= 32) {
            ...
            /* Check static objects in current quadrant */
            QuadrantIndex *lq = &spatial_index[pq1][pq2][pq3];
...
void handle_scan(int i, const char *params) {
    int tid;
    if (sscanf(params, " %d", &tid) == 1) {
        if (players[i].state.system_health[2] < 20.0f) {
        ...
        int pq1 = players[i].state.q1, pq2 = players[i].state.q2, pq3 = players[i].state.q3;
        float sensor_h = players[i].state.system_health[2];
        bool scrambled = (sensor_h < 50.0f && (rand() % 100 > (int)sensor_h));
...
void handle_clo(int i, const char *params) {
    if (!players[i].state.is_cloaked) {
        ...
        if (players[i].state.system_health[9] < 50.0f && (rand() % 100 > (int)players[i].state.system_health[9])) {
...
void handle_bor(int i, const char *params) {
    ...
    if (tid > 0) {
        ...
        if (players[i].state.system_health[3] < 20.0f) {
        ...
        /* Resolve target position and validity */
        if (tid >= 1 && tid <= 32 && players[tid-1].active && players[tid-1].state.q1 == pq1 && players[tid-1].state.q2 == pq2 && players[tid-1].state.q3 == pq3) { tx=players[tid-1].state.s1; ty=players[tid-1].state.s2; tz=players[tid-1].state.s3; found=true; }
        else if (tid >= 1000 && tid < 1000+MAX_NPC && npcs[tid-1000].active && npcs[tid-1000].q1 == pq1 && npcs[tid-1000].q2 == pq2 && npcs[tid-1000].q3 == pq3) { tx=npcs[tid-1000].x; ty=npcs[tid-1000].y; tz=npcs[tid-1000].z; found=true; }
...
void handle_dis(int i, const char *params) {
    ...
    if (tid > 0) {
        ...
        if (players[i].state.system_health[3] < 15.0f) {
...
void handle_min(int i, const char *params) {
    ...
    if (target_idx != -1) {
        ...
        float yield_mult = 0.8f + (players[i].state.system_health[3] / 100.0f) * 0.2f;
...
void handle_sco(int i, const char *params) {
    ...
    if(near) { 
        ...
        float aux_h = players[i].state.system_health[9];
        int gain = (int)(5000 * (0.8f + (aux_h / 100.0f) * 0.2f));
        ...
        if (players[i].state.shields[s_idx] >= 500) {
            ...
            float hull_dmg = 2.0f + (rand() % 300) / 100.0f;
            players[i].state.hull_integrity -= hull_dmg;
            players[i].state.system_health[7] -= 5.0f; /* Life Support damage (ID 7) */
            send_server_msg(i, "CRITICAL", "WARNING: Solar heat bypassing shields! Hull and Life Support damaged!");
        }
        
        char msg[128];
        sprintf(msg, "Solar energy harvested: %d units stored in Cargo Bay. Collectors efficiency: %.0f%%.", gain, (aux_h/100.0f)*100);
        send_server_msg(i, "ENGINEERING", msg); 
    } 
...
void handle_har(int i, const char *params) {
    ...
    if (players[i].state.system_health[9] < 25.0f) {
    ...
    if(near) { 
        ...
        float aux_h = players[i].state.system_health[9];
        float efficiency = 0.6f + (aux_h / 100.0f) * 0.4f;
        ...
        if (players[i].state.shields[s_idx] >= 1000) {
            ...
            players[i].state.shields[s_idx] = 0;
            float hull_dmg = 5.0f + (rand() % 500) / 100.0f;
            players[i].state.hull_integrity -= hull_dmg;
            
            /* Heavy damage to critical systems: Sensors(2), Computer(6), Life Support(7) */
            players[i].state.system_health[2] -= (5.0f + (rand() % 10));
            players[i].state.system_health[6] -= (5.0f + (rand() % 10));
            players[i].state.system_health[7] -= (5.0f + (rand() % 10));
...
void handle_doc(int i, const char *params) {
    ...
    for(int b=0; b<MAX_BASES; b++) {
        if(bases[b].active && bases[b].q1==players[i].state.q1 && bases[b].q2==players[i].state.q2 && bases[b].q3==players[i].state.q3) {
            ...
            if(d <= (DIST_DOCKING_MAX + 0.05f)) { base_idx = b; break; } 
        }
    }
    ...
    if(base_idx != -1) {
        ...
        if (players[i].state.system_health[1] < 10.0f || players[i].state.system_health[9] < 10.0f) {
...
void handle_load(int i, const char *params) {
    ...
    if (players[i].state.system_health[9] < 10.0f) {
...
void handle_rep(int i, const char *params) {
    ...
    if(sscanf(params, " %d", &sid) == 1) {
        if (sid >= 0 && sid < 10) {
            ...
            if (players[i].state.system_health[6] < 10.0f && players[i].state.system_health[9] < 10.0f && sid != 6 && sid != 9) {
        ...
        for(int s=0; s<10; s++) {
            char line[128];
            float hp = players[i].state.system_health[s];
...
void handle_fix(int i, const char *params) {
    ...
    float efficiency = 10.0f + (players[i].state.system_health[9] / 100.0f) * 10.0f;
...
void handle_sta(int i, const char *params) {
    ...
    if (players[i].state.system_health[6] < 5.0f) {
    ...
    bool scrambled = (comp_h < 30.0f && (rand() % 100 > (int)comp_h));
    ...
    sprintf(b+strlen(b), " HEADING:  %03.0f        MARK:   %+03.0f\n", players[i].state.van_h, players[i].state.van_m);

    strcat(b, BLUE "\n[ POWER AND REACTOR STATUS ]\n" RESET);
    float en_pct = (players[i].state.energy / 1000000.0f) * 100.0f;
    ...
    for(int s=0; s<10; s++) { 
        float hp = players[i].state.system_health[s]; 
        const char* col = (hp > 75) ? GREEN : (hp > 25) ? YELLOW : RED; 
        sprintf(b+strlen(b), " %-10s: %s%5.1f%%" RESET " ", sys[s], col, hp); 
...
void handle_who(int i, const char *params) {
    ...
    for (int j = 0; j < MAX_CLIENTS; j++) {
        if (players[j].active) {
            ...
            bool scrambled = (comp_h < 40.0f && q_dist > (comp_h / 4.0f));
...
void handle_jum(int i, const char *params) {
    ...
    if (sscanf(params, "%d %d %d", &qx, &qy, &qz) == 3) {
        ...
        if (players[i].state.system_health[0] < 75.0f && (rand() % 100 > (int)players[i].state.system_health[0])) {
        ...
        float stress = 1.0f + (rand() % 200) / 100.0f;
        players[i].state.hull_integrity -= stress;
        ...
        if (players[i].state.system_health[2] < 100.0f) {
            float noise = (100.0f - players[i].state.system_health[2]) / 20.0f; /* Up to 2.5 units off */
            off_x = ((rand() % 2000 - 1000) / 1000.0f) * noise;
            off_y = ((rand() % 2000 - 1000) / 1000.0f) * noise;
            off_z = ((rand() % 2000 - 1000) / 1000.0f) * noise;
        }
...
void handle_psy(int i, const char *params) {
    ...
    if (players[i].state.system_health[6] < 20.0f) {
...
void handle_map(int i, const char *params) {
    if (players[i].state.system_health[6] < 15.0f) {
...
void handle_aux(int i, const char *params) {
    ...
    if(strncmp(p_ptr, "jettison", 8) == 0) {
        ...
        if (players[i].state.energy < 100) {
            send_server_msg(i, "ENGINEERING", "Insufficient energy for emergency core ejection (Req: 100).");
            return;
        }
        ...
        if (players[i].state.system_health[2] < 25.0f || players[i].state.system_health[6] < 25.0f) {
        ...
        int qx, qy, qz;
        if (sscanf(p_ptr + 5, "%d %d %d", &qx, &qy, &qz) == 3) {
            ...
            float time = dist / 3.33f; if (time < 1.0f) time = 1.0f;
            
            players[i].state.probes[p_idx].vx = dx / (time * 30.0f);
            players[i].state.probes[p_idx].vy = dy / (time * 30.0f);
            players[i].state.probes[p_idx].vz = dz / (time * 30.0f);
        ...
        if (sscanf(p_ptr + 6, "%d", &p_idx) == 1) {
            ...
            sprintf(msg, "\n" CYAN ".--- PROBE P%d: REAL-TIME DEEP SPACE TELEMETRY ---." RESET "\n"
                         ...
                         lq->planet_count, lq->bh_count, lq->nebula_count, lq->pulsar_count,
